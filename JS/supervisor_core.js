...
let empCache = [];
let periodos = (typeof generarPeriodos === 'function') ? generarPeriodos() : [];
...
window.normalizarRegistroDesdeTimestamp = function (r) {
...
function calcularNetWorkedOrdinario(periodosDia, esFestivo) {
  ...
  if (Array.isArray(periodosDia)) {
    periodosDia.forEach(p => {
      ...
      let ordE = Math.max(HORA_ENTRADA_REF, mE);
      let ordS = Math.min(HORA_SALIDA_REF, mS);
      if (ordS > ordE) {
        ordBruto += (ordS - ordE);
      }
    });
...
function filtrarResumenMensual() {
  ...
  const rangoInfo = (typeof window.obtenerRangoFechasReportes === 'function')
    ? window.obtenerRangoFechasReportes()
    : null;
  let idx = parseInt($('periodoMensual')?.value || 0);
  let periodoSel = rangoInfo?.periodo || periodos[idx];
  if (periodoSel) {
    const R_INI = rangoInfo ? rangoInfo.R_INI : periodoSel.inicio;
    const R_FIN = rangoInfo ? rangoInfo.R_FIN : periodoSel.fin;
...
window.obtenerFechasPendientesRegularizarEmpleado = function (emp, customInicio = null, customFin = null) {
  ...
  fechasRango.forEach(f => {
    ...
    if (!isJustificado) {
      let netWorkedOrdinario = (typeof calcularNetWorkedOrdinario === 'function') ? calcularNetWorkedOrdinario(periodosDia, false) : netWorked;
      let missingMinutes = Math.max(0, 480 - netWorkedOrdinario);
...
function cargarAsistencia() {
  ...
  window._asisData = empCache.map(e => {
    ...
    let modoBadge = '';
    if (modo === 'CAMPO') {
      modoBadge = '<span class="rep-badge-pill rep-badge-campo"><i class="fas fa-truck-pickup" style="font-size:9.5px;"></i> CAMPO</span>';
    } else if (modo === 'EMPRESA' || modo === 'PLANTA') {
      modoBadge = '<span class="rep-badge-pill" style="background:#f1f5f9; color:#334155; border:1px solid #cbd5e1;"><i class="fas fa-building" style="font-size:9.5px;"></i> PLANTA</span>';
    } else {
      modoBadge = '<span class="rep-asis-empty">—</span>';
    }
    ...
    if ((eReg?.autoriza || '').includes('CAMPO')) {
      ...
      extrasBadge = '<span class="rep-badge-pill" style="color:#94a3b8; background:#f8fafc; border:1px solid #e2e8f0; font-weight:600;">NO</span>';
    }

    // Hacer modo y extras editables para Admin y Sup Admin
    let modoHtml = modoBadge;
    let extrasHtml = extrasBadge;
    if (canEditAttendance) {
      const clickModo = `onclick="event.stopPropagation();editarValorRegistro('${e.id}', 'ENTRADA', '${eReg?.id || ''}', 'modo', '${modo}', '${hoy}')"`;
      const clickExtras = `onclick="event.stopPropagation();editarValorRegistro('${e.id}', 'ENTRADA', '${eReg?.id || ''}', 'horasExtra', '${extrasVal}', '${hoy}')"`;
      modoHtml = `<span class="editable-pill" ${clickModo} title="Clic para cambiar modo">${modoBadge}</span>`;
      extrasHtml = `<span class="editable-pill" ${clickExtras} title="Clic para editar extras">${extrasBadge}</span>`;
    }
    ...
    if (!e.entradaHoy && !isSinAsistencia) {
      ...
      toggle = `
        <div class="almuerzo-toggle-modern">
          <button type="button" class="almuerzo-btn-toggle ${isSi ? 'active-si' : ''} ${!puedeEditar ? 'disabled' : ''}" 
                  onclick="event.stopPropagation();cambiarEstadoAlmuerzo('${e.id}','SI')" 
                  ${!puedeEditar ? 'disabled' : ''} 
                  title="Almuerzo en Planta">
            <i class="fas fa-utensils"></i> Sí
          </button>
          <button type="button" class="almuerzo-btn-toggle ${isNo ? 'active-no' : ''} ${!puedeEditar ? 'disabled' : ''}" 
                  onclick="event.stopPropagation();cambiarEstadoAlmuerzo('${e.id}','NO')" 
                  ${!puedeEditar ? 'disabled' : ''} 
                  title="Sin Almuerzo">
  ...
  extrasHoyTb.forEach((extra, idx) => {
    window._asisData.push({
      ...
      foto_url: '',
      _eH: `<span class="rep-badge-pill" style="background:#f1f5f9; color:#64748b; border:1px solid #e2e8f0; font-size:10.5px;"><i class="fas fa-clock" style="font-size:9px;"></i> ${extra.hora || '--:--'}</span>`,
      _sH: `<span class="rep-asis-empty">—</span>`,
      _modo: `<span class="rep-asis-empty">N/A</span>`,
      _extras: `<span class="rep-asis-empty">N/A</span>`,
      _est: `<span class="rep-badge-pill" style="background:#f0fdf4; color:#15803d; border:1px solid #bbf7d0; font-size:10.5px;"><i class="fas fa-id-badge"></i> Invitado</span>`,
      _ausencia: `<span class="rep-asis-empty">—</span>`,
      _toggle: `<span class="rep-badge-pill" style="background:#eff6ff; color:#1e40af; border:1px solid #bfdbfe; font-size:11px; font-weight:700;"><i class="fas fa-utensils" style="margin-right:4px;"></i> +${extra.cantidad || 1} Extra(s)</span>`,
      _entradaHoy: false,
      _salidaHoy: false,
      _tard: false,
      _almuerzoHoy: 'SI',
...
function filtrarAsistenciaTabla() {
  ...
  const mult = (typeof _sortAsisDir !== 'undefined' && _sortAsisDir === 'desc') ? -1 : 1;
  data.sort((a, b) => mult * (a.nombre || '').localeCompare(b.nombre || '', 'es', { sensitivity: 'base' }));
  if (!data.length) {
    $('asistenciaTablaContainer').innerHTML = '<div class="empty-state" style="padding:45px 20px; text-align:center;"><i class="fas fa-users-slash" style="font-size:32px; color:#cbd5e1; margin-bottom:12px;"></i><p style="color:#64748b; font-weight:600; font-size:13px;">No hay colaboradores que coincidan con el filtro seleccionado</p></div>';
    return;
  }
  ...
  const sortIcon = (typeof _sortAsisDir !== 'undefined' && _sortAsisDir === 'desc') ? 'fa-sort-alpha-up-alt' : 'fa-sort-alpha-down';
  const colDefs = {
    'Empleado': {
      ...
      footer: `<td><strong style="color:#38bdf8; font-size:12px; display:inline-flex; align-items:center; gap:5px;"><i class="fas fa-users" style="color:#38bdf8;"></i> TOTALES (${totalVisible})</strong></td>`
    },
    'Área': {
      header: `<th style="background:#f8fafc; color:#475569; border-bottom:2px solid #64748b;"><i class="fas fa-building" style="color:#64748b; margin-right:5px;"></i> Área</th>`,
      body: e => `<td><span class="rep-asis-chip"><i class="fas fa-briefcase" style="font-size:9px; color:#94a3b8;"></i> ${escapeHtml(e.area || '—')}</span></td>`,
      footer: `<td style="color:#94a3b8;">—</td>`
    },
    'Regularización': {
    ...
    'Entrada': {
      ...
      body: e => `<td style="text-align:center;">${e._eH}</td>`,
      footer: `<td style="text-align:center;"><span class="rep-badge-pill" style="background:#064e3b; color:#4ade80; border:1px solid #047857;"><i class="fas fa-sign-in-alt" style="font-size:9.5px;"></i> ${countEntradas} Entradas</span></td>`
    },
    'Salida': {
      header: `<th style="background:#f0f9ff; color:#0284c7; border-bottom:2px solid #0284c7; text-align:center;"><i class="fas fa-sign-out-alt" style="color:#0284c7; margin-right:5px;"></i> Salida</th>`,
      body: e => `<td style="text-align:center;">${e._sH}</td>`,
      footer: `<td style="text-align:center;"><span class="rep-badge-pill" style="background:#0c4a6e; color:#38bdf8; border:1px solid #0284c7;"><i class="fas fa-sign-out-alt" style="font-size:9.5px;"></i> ${countSalidas} Salidas</span></td>`
    },
    'Modo': {
      header: `<th style="background:#ecfeff; color:#0891b2; border-bottom:2px solid #0891b2; text-align:center;"><i class="fas fa-laptop-house" style="color:#0891b2; margin-right:5px;"></i> Modo</th>`,
      body: e => `<td style="text-align:center;">${e._modo}</td>`,
      footer: `<td style="text-align:center;"><span class="rep-badge-pill" style="background:#164e63; color:#67e8f9; border:1px solid #0891b2;"><i class="fas fa-truck-pickup" style="font-size:9.5px;"></i> ${countCampoVis} Campo</span></td>`
    },
    'Extras': {
      header: `<th style="background:#eff6ff; color:#1d4ed8; border-bottom:2px solid #1d4ed8; text-align:center;"><i class="fas fa-bolt" style="color:#1d4ed8; margin-right:5px;"></i> Extras</th>`,
      body: e => `<td style="text-align:center;">${e._extras}</td>`,
      footer: `<td style="text-align:center;"><span class="rep-badge-pill" style="background:#1e3a8a; color:#93c5fd; border:1px solid #1d4ed8;"><i class="fas fa-bolt" style="font-size:9.5px;"></i> ${countExtrasAut} Aut.</span></td>`
    },
    'Estado': {
      header: `<th style="background:#f0fdf4; color:#15803d; border-bottom:2px solid #15803d; text-align:center;"><i class="fas fa-shield-alt" style="color:#15803d; margin-right:5px;"></i> Estado</th>`,
      body: e => `<td style="text-align:center;">${e._est}</td>`,
      footer: `<td style="text-align:center; color:#94a3b8;">—</td>`
    },
    'Razón Ausencia': {
      header: `<th style="background:#faf5ff; color:#7c3aed; border-bottom:2px solid #7c3aed; text-align:center;"><i class="fas fa-file-signature" style="color:#7c3aed; margin-right:5px;"></i> Razón Ausencia</th>`,
      body: e => `<td style="text-align:center;">${e._ausencia}</td>`,
      footer: `<td style="text-align:center; color:#94a3b8;">—</td>`
    },
    'Almuerzo': {
      header: `<th style="background:#f0f9ff; color:#0284c7; border-bottom:2px solid #0284c7; text-align:center;"><i class="fas fa-utensils" style="color:#0284c7; margin-right:5px;"></i> Almuerzo</th>`,
      body: e => `<td style="text-align:center;">${e._toggle}</td>`,
      footer: `<td style="text-align:center;"><span class="rep-badge-pill" style="background:#713f12; color:#fde047; border:1px solid #a16207;"><i class="fas fa-utensils" style="font-size:9.5px;"></i> ${countAlmPlanta} Planta</span></td>`
    }
  };
  ...
  let footerHtml = `<tr class="rep-totals-row">${footerCellsHtml}</tr>`;
...
window.sortAsistencia = sortAsistencia;
...
window.obtenerListaEmpleadosReportes = function () {
  ...
  const baseEmps = (empCache && empCache.length) ? empCache : (window.empCache || []);
  let lista = [...baseEmps];
  if (verBajas) {
...
window.obtenerRangoFechasReportes = function () {
  const selectPeriodo = $('periodoMensual') || $('periodoMensualDash') || $('kpiDetallePeriodo');
  const idx = parseInt(selectPeriodo?.value || 0, 10);
  const listaPeriodos = (periodos && periodos.length) ? periodos : ((typeof generarPeriodos === 'function') ? generarPeriodos() : []);
  const periodo = (listaPeriodos && listaPeriodos[idx]) ? listaPeriodos[idx] : (listaPeriodos ? listaPeriodos[0] : null);

  let inpIni = $('filtroFechaReportesInicio')?.value?.trim();
  let inpFin = $('filtroFechaReportesFinalizacion')?.value?.trim();

  // Fallback a filtro individual de dashboard si aplica
  const inpDash = $('filtroFechaDashboard')?.value?.trim();
  if (!inpIni && !inpFin && inpDash) {
    inpIni = inpDash;
    inpFin = inpDash;
  }

  // Si solo se ingresó una de las dos fechas, unificar para rango de 1 solo día
  if (inpIni && !inpFin) inpFin = inpIni;
  if (!inpIni && inpFin) inpIni = inpFin;

  let R_INI = inpIni || (periodo ? periodo.inicio : '');
  let R_FIN = inpFin || (periodo ? periodo.fin : '');

  // Validar y corregir si el usuario invirtió fechas (inicio > fin)
  if (inpIni && inpFin && inpIni > inpFin) {
    const tmp = R_INI;
    R_INI = R_FIN;
    R_FIN = tmp;
    if ($('filtroFechaReportesInicio')) $('filtroFechaReportesInicio').value = R_INI;
    if ($('filtroFechaReportesFinalizacion')) $('filtroFechaReportesFinalizacion').value = R_FIN;
    if (typeof mostrarToast === 'function') {
      mostrarToast(`Rango invertido corregido: ${formatearFechaA_DMY(R_INI)} al ${formatearFechaA_DMY(R_FIN)}`, 'info');
    }
  }
...
function cargarReportes() {
  ...
  const R_INI = rango.R_INI;
  const R_FIN = rango.R_FIN;
  const emps = (empCache && empCache.length) ? empCache : (window.empCache || []);
  if (!R_INI || !R_FIN || (!emps.length && !window.empEliminadosCache?.length && !window._cacheDesvinculados?.length)) return;
  ...
  if ($('repFaltas')) $('repFaltas').textContent = totalFaltas;
  if ($('repPermisoMedico')) $('repPermisoMedico').textContent = formatearMinutos(totalPermisoMedico);
  ...
  if ($('repHorasExtra50')) $('repHorasExtra50').textContent = formatearHorasDecimal(totalHorasExtra50);
  if ($('repHorasExtra100')) $('repHorasExtra100').textContent = formatearHorasDecimal(totalHorasExtra100);
  ...
  _reportesCustomData = stats;
  window._reportesCustomData = stats;
...
window.aplicarPresetFechasReporte = function (preset) {
  const selectPeriodo = $('periodoMensual') || $('periodoMensualDash');
  const idx = parseInt(selectPeriodo?.value || 0, 10);
  const periodo = (periodos && periodos[idx]) ? periodos[idx] : (periodos ? periodos[0] : null);
  const inpIni = $('filtroFechaReportesInicio');
  const inpFin = $('filtroFechaReportesFinalizacion');
  const btnLimpiar = $('btnLimpiarFechaRep');

  if (!periodo) return;

  if (preset === 'periodo' || preset === 'todo') {
    if (inpIni) inpIni.value = '';
    if (inpFin) inpFin.value = '';
    if (btnLimpiar) btnLimpiar.style.display = 'none';
  } else if (preset === 'quincena1') {
    if (inpIni) inpIni.value = periodo.inicio;
    const finParts = periodo.fin.split('-');
    if (inpFin) inpFin.value = `${finParts[0]}-${finParts[1]}-10`;
    if (btnLimpiar) btnLimpiar.style.display = 'inline-block';
  } else if (preset === 'quincena2') {
    const finParts = periodo.fin.split('-');
    if (inpIni) inpIni.value = `${finParts[0]}-${finParts[1]}-11`;
    if (inpFin) inpFin.value = periodo.fin;
    if (btnLimpiar) btnLimpiar.style.display = 'inline-block';
  } else if (preset === 'hoy') {
    const hoy = getLocalHoyStr();
    if (inpIni) inpIni.value = hoy;
    if (inpFin) inpFin.value = hoy;
    if (btnLimpiar) btnLimpiar.style.display = 'inline-block';
  }
...
window.cargarReportesConFiltroFecha = function () {
  ...
  const hasValue = (inpIni && inpIni.value) || (inpFin && inpFin.value) || (inputFechaDash && inputFechaDash.value);
  if (hasValue) {
...
window.syncPeriodo = function (source) {
  ...
  // Restablecer filtros de fechas personalizadas para enfocar el nuevo período seleccionado
  if ($('filtroFechaReportesInicio')) $('filtroFechaReportesInicio').value = '';
  if ($('filtroFechaReportesFinalizacion')) $('filtroFechaReportesFinalizacion').value = '';
...
window.syncFecha = function (source) {
  if (source === 'dash') {
    ...
    const inpFin = $('filtroFechaReportesFinalizacion');
    if (val) {
      if (inpIni) inpIni.value = val;
      if (inpFin) inpFin.value = val;
    }
    const btnLimpiarRep = $('btnLimpiarFechaRep');
    const btnLimpiarDash = $('btnLimpiarFechaDash');
    if (val) {
...
function filtrarTablaReportes() {
  ...
  // Construir headers según columnas visibles con acentos y categorías
  let headerHtml = `<th onclick="sortarTablaReportes('nombre')" style="cursor:pointer; min-width:220px; background:#f8fafc; border-bottom:2.5px solid #2563eb;">
  ...
  COLUMNAS_DISPONIBLES.forEach(col => {
    if (columnasVisibles.includes(col.id)) {
      ...
      if (col.id === 'area') {
        headerHtml += `<th onclick="sortarTablaReportes('area')" style="cursor:pointer; min-width:140px; background:#f8fafc; border-bottom:2.5px solid #64748b;">
          <div style="display:flex; align-items:center; gap:5px;">
            ${iconHtml}<span style="font-weight:700; color:#334155;">ÁREA</span> ${sortIcon('area')}
          </div>
        </th>`;
      } else {
        headerHtml += `<th onclick="sortarTablaReportes('${col.id}')" style="text-align:center; cursor:pointer; min-width:115px; background:${col.colorBg || '#f8fafc'}; border-bottom:2.5px solid ${col.color || '#2563eb'};">
          <div style="display:flex; align-items:center; justify-content:center; gap:4px;" title="${col.catLabel || col.label}">
            ${iconHtml}<span style="color:${col.color || '#1e293b'}; font-weight:700;">${col.label}</span> ${sortIcon(col.id)}
          </div>
        </th>`;
      }
    }
      ...
      if (columnasVisibles.includes(col.id)) {
        ...
        if (col.id === 'area') {
          ...
          if (col.id === 'asistencias') {
            ...
            if (col.tipo === 'tiempo') {
              contenido = `<span style="font-family:'Fira Code',monospace; font-size:11px; font-weight:600; color:#334155;">${minutosAHHMMSS(valor)}</span>`;
            } else {
              contenido = `<span style="font-weight:700; color:#1e293b;">${valor}</span>`;
            }
          }
  ...
  // Fila de totales sticky para tabla mensual
...
async function mostrarDetalle(id, indexPeriodo = 0, customInicio = null, customFin = null, fechaEnfocar = null) {
      ...
      try {
        ...
        if (Array.isArray(fullRegs) && fullRegs.length > 0) {
          ...
          (e.registros || []).forEach(r => {
            ...
            existingMap.set(`${fNorm}_${tipoUpper}_${hNorm}`, r);
            existingMap.set(`${fNorm}_${tipoUpper}`, r);
          ...
          fullRegs.forEach(r => {
            ...
            const hNorm = (r.hora || '').slice(0, 5);
            const tipoUpper = (r.tipo || '').toUpperCase();
            const kExact = `${fNorm}_${tipoUpper}_${hNorm}`;
            const kTipo = `${fNorm}_${tipoUpper}`;
            const existing = (r.id && existingMap.get(String(r.id))) || existingMap.get(kExact) || existingMap.get(kTipo);
            if (!existing) {
              if (!e.registros) e.registros = [];
              e.registros.push({ ...r, fecha: fNorm });
              existingMap.set(kExact, r);
              existingMap.set(kTipo, r);
              if (r.id) existingMap.set(String(r.id), r);
              hasChanges = true;
            } else {
              ...
              if (r.razon_ausencia && !existing.razon_ausencia) {
                existing.razon_ausencia = r.razon_ausencia;
                hasChanges = true;
              }
              if (r.razon_justificac && !existing.razon_justificac) {
                existing.razon_justificac = r.razon_justificac;
                hasChanges = true;
              }
  ...
  function rebuildTable() {
    ...
    [...regs].sort((a, b) => {
      ...
      const rH = (r.hora || '').slice(0, 5); // HH:mm
      
      // Evitar registros duplicados para el mismo día, tipo y hora (mismo minuto)
      const yaExiste = porDia[fechaNorm].registros.some(ex => {
        ...
        const esSalidaEx = exTipo.includes('SALIDA');
        const esSalidaR = rTipo.includes('SALIDA');
        if (esSalidaEx && esSalidaR && exH && rH && exH === rH) return true;
        const esEntradaEx = exTipo.includes('ENTRADA') || exTipo.includes('RETORNO');
        const esEntradaR = rTipo.includes('ENTRADA') || rTipo.includes('RETORNO');
        if (esEntradaEx && esEntradaR && exH && rH && exH === rH) return true;
        return false;
      });
      ...
      if (!yaExiste) {
        ...
        if (ex) {
          ...
          if (r.permiso_personal_mins) ex.permiso_personal_mins = r.permiso_personal_mins;
          if (r.permiso_medico_mins) ex.permiso_medico_mins = r.permiso_medico_mins;
          if (r.tiempo_justificado_mins) ex.tiempo_justificado_mins = r.tiempo_justificado_mins;
          if (r.razon_permiso) ex.razon_permiso = r.razon_permiso;
          if (r.razon_ausencia && !ex.razon_ausencia) ex.razon_ausencia = r.razon_ausencia;
          if (r.razon_justificac && !ex.razon_justificac) ex.razon_justificac = r.razon_justificac;
        }
      }
    ...
    let filas = fechasOrdenadas.map(f => {
      ...
      regsDia.forEach(r => {
        ...
        if (tipo === 'ENTRADA' || tipo === 'RETORNO_CAMPO' || tipo === 'ENTRADA_CAMPO') {
          ...
          if (entradaPendiente) {
            let mPend = obtenerMinutos(entradaPendiente.timestamp || entradaPendiente.hora);
            if (mPend !== null && mE !== null && Math.abs(mE - mPend) <= 2) {
              return; // Entrada duplicada consecutiva, ignorar
            }
          }
          ...
          if (entradaPendiente) {
            ...
            // Protección contra salidas duplicadas consecutivas: si el último período ya tiene salida con hora idéntica (dentro de 2 minutos)
            const ultP = periodosDia[periodosDia.length - 1];
            if (ultP && ultP.salida) {
              let mUlt = obtenerMinutos(ultP.salida.timestamp || ultP.salida.hora);
              let mCur = obtenerMinutos(r.timestamp || r.hora);
              if (mUlt !== null && mCur !== null && Math.abs(mCur - mUlt) <= 2) {
                // Es un duplicado evidente de la salida anterior, ignorar y no crear fila fantasma
                return;
              }
            }
            periodosDia.push({ entrada: null, salida: r });
      ...
      let horaE = periodosDia.map(p => {
        return p.entrada ? formatearHora(p.entrada.timestamp || p.entrada.hora) : '--:--';
      }).join('<br>');
      ...
      if (esPrevioAlRegistro) {
        ...
        <td colspan="8" style="font-size:10.5px; padding:6px 12px; background:${cardBg}; border-left:3px solid ${cardBorder}; cursor:pointer;" onclick="window.abrirModalGestionJornada('${targetEmpId}', '${f}')">
      ...
      // Badges consolidados de Novedades y Permisos (sin redundancia)
      let badgesNovedades = [];
      if (atrasoMins > 0) {
        badgesNovedades.push(`<span class="badge-tiempo badge-atraso" title="Atraso al ingreso: ${minutosAHHMMSS(atrasoMins)}"><i class="fas fa-clock" style="font-size:9px;"></i> Atraso: ${minutosAHHMMSS(atrasoMins)}</span>`);
      }
      ...
      if (tiempoPersonal > 0) {
        badgesNovedades.push(`<span class="badge-tiempo badge-pers" title="Permiso Personal: ${minutosAHHMMSS(tiempoPersonal)}"><i class="fas fa-user" style="font-size:9px;"></i> TP: ${minutosAHHMMSS(tiempoPersonal)}</span>`);
      }
      if (tiempoMedico > 0) {
      ...
      const celdaNovedadesHtml = badgesNovedades.length > 0
        ? `<div style="display:flex; align-items:center; justify-content:center; gap:4px; flex-wrap:wrap;">${badgesNovedades.join(' ')}</div>`
        : '<span style="color:#94a3b8; font-size:11px;">Normal</span>';

      const totalH50Dia = h50 + hC50;
      const totalH100Dia = h100 + hC100;

      const celdaH50Html = totalH50Dia > 0 ? `
      ...
      <td style="white-space:nowrap; font-weight:600; font-size:10.5px; padding:6px 8px; cursor:pointer;" onclick="event.stopPropagation(); window.abrirModalGestionJornada('${targetEmpId}', '${f}')">${fechaFormateada}</td>
      <td class="hora-cell" style="font-size:11.5px; font-weight:600; padding:6px 8px; text-align:center;">${horaE}</td>
      <td class="hora-cell" style="font-size:11.5px; font-weight:600; padding:6px 8px; text-align:center;">
        ${horaS}
        ${modActual !== 'EMPRESA' ? `<div style="margin-top:2px;">${modalidadCell}</div>` : ''}
      </td>
      <td style="text-align:center; color:${netWorked > 0 ? '#15803d' : '#94a3b8'}; font-weight:700; font-size:11.5px; padding:6px 8px;">${netWorked > 0 ? minutosAHHMMSS(netWorked) : '—'}</td>
    ...
    const tfootRow = `<tr style="background:var(--g50); border-top:2px solid var(--g300); font-weight:700; font-size:11px;">
        <td style="padding:6px 8px; font-weight:800; color:#1e293b;">TOTALES</td>
        <td style="text-align:center; color:#94a3b8;">—</td>
        <td style="text-align:center; color:#94a3b8;">—</td>
        <td style="text-align:center; color:#15803d; font-weight:800; padding:6px 8px;">${mA(totHoras)}</td>
        <td style="text-align:center; color:#94a3b8;">—</td>
        <td style="text-align:center; padding:6px 8px;">
          <div style="display:flex; flex-wrap:wrap; justify-content:center; gap:5px; font-size:10px;">
            ${totAtrasos > 0 ? `<span style="background:#fee2e2; color:#b91c1c; padding:2px 6px; border-radius:5px; border:1px solid #fca5a5;" title="Total Atrasos">Atrasos: <strong>${minutosAHHMMSS(totAtrasos)}</strong></span>` : ''}
        ...
        <td style="text-align:center; padding:6px 8px;">${celdaTotalDescuentoHtml}</td>
        <td style="text-align:center; padding:6px 8px; color:#1d4ed8; font-weight:800;">${mA(totExtra50)}</td>
        <td style="text-align:center; padding:6px 8px; color:#4338ca; font-weight:800;">${mA(totExtra100)}</td>
      </tr>`;

    const tbody = document.getElementById('tbody-historial-periodo');
    if (tbody) {
      tbody.innerHTML = filas || '<tr><td colspan="9" class="empty-state">Sin registros</td></tr>';
    }
  ...
  function actualizarCardVacaciones(vacacionesTomadasHoy, vacacionesRestantesHoy) {
    ...
    if (container) {
      ...
      if (vacacionesList.length > 0) {
        ...
        listaVacacionesHTML = `<div style="font-size:10.5px; color:#94a3b8; text-align:center; padding:4px 0;">Sin vacaciones registradas</div>`;
      }

      container.innerHTML = `
        <div class="kpi-header-row">
          <span class="kpi-header-title" style="font-size:10.5px;">
            <i class="fas fa-umbrella-beach" style="color:#d97706; font-size:11.5px;"></i> Vacaciones
          </span>
          <span class="emp-chip" style="background:#fffbeb; color:#b45309; font-size:9.5px; padding:1px 6px; white-space:nowrap;">
            ${totalVacsTomadas} tom. / <strong style="color:var(--green);">${totalVacsRestantes}</strong> disp.
          </span>
        </div>
        <div class="kpi-card-stats-grid">
          <div class="kpi-stat-item">
            <span class="kpi-stat-label">Tomadas</span>
            <div class="kpi-stat-val-hero" style="color:#d97706;">${totalVacsTomadas} <span style="font-size:10px; font-weight:500; color:#94a3b8;">días</span></div>
          </div>
          <div class="kpi-stat-item">
            <span class="kpi-stat-label">Disponibles</span>
            <div class="kpi-stat-val-hero" style="color:var(--green);">${totalVacsRestantes} <span style="font-size:10px; font-weight:500; color:#94a3b8;">días</span></div>
          </div>
          <div style="grid-column: span 2; border-top: 1px dashed #f1f5f9; padding-top: 4px; margin-top: 2px; max-height: 44px; overflow-y: auto; display: flex; flex-direction: column; gap: 2px;">
            ${listaVacacionesHTML}
          </div>
        </div>
      `;
  ...
  if (tieneWa) {
    badgeWhatsAppHtml = `
      <button type="button" class="btn-action-detail btn-action-wa" onclick="window.abrirModalMensajeIndividualWhatsApp('${e.id}')" title="Enviar WhatsApp a ${escapeHtml(e.nombre)} (${escapeHtml(rawTel)})">
        <i class="fab fa-whatsapp" style="font-size:13px;"></i> <span>WhatsApp: ${escapeHtml(rawTel)}</span>
      </button>
    `;
  } else {
    badgeWhatsAppHtml = `
      <button type="button" class="btn-action-detail btn-action-wa-add" onclick="window.abrirModalMensajeIndividualWhatsApp('${e.id}')" title="Registrar número para enviar WhatsApp">
        <i class="fab fa-whatsapp" style="font-size:13px;"></i> <span>+ WhatsApp</span>
      </button>
    `;
  }
  ...
  const badgeCulturaHtml = `
    <button type="button" class="btn-action-detail ${culturaActivaEmp ? 'btn-action-cultura-on' : 'btn-action-cultura-off'}" onclick="window.toggleCulturaEmpleado('${e.id}')" title="${culturaActivaEmp ? 'Cultura Tcontrol Habilitada para este colaborador. Clic para deshabilitar / exonerar' : 'Cultura Tcontrol Deshabilitada para este colaborador. Clic para habilitar'}">
      <i class="fas fa-lightbulb"></i>
      <span>Cultura: <strong>${culturaActivaEmp ? 'Habilitado' : 'Exonerado'}</strong></span>
      <i class="fas fa-rotate" style="font-size:9px; opacity:0.6;"></i>
    </button>
  `;

  $('detalleContent').innerHTML = `
    <div class="detail-view-modern">
      <!-- CABECERA REDISEÑADA: HERO PROFILE CARD -->
      <div class="detail-hero-card">
        <!-- TOP: IDENTITY & ACTION BUTTONS -->
        <div class="detail-hero-main">
          <div class="detail-identity-group">
            <div class="detail-avatar-wrapper" ${esMaster ? `onclick="triggerPhotoUpload('${e.id}')" title="Subir o cambiar fotografía"` : ''}>
              ${photoCell(e, 'large')}
              ${esMaster ? `<div class="detail-avatar-hover"><i class="fas fa-camera"></i></div>` : ''}
            ...
            <div class="detail-identity-info">
              ...
              <div class="detail-chips-row">
                ...
                <span class="emp-chip ${e.id_dispositivo ? 'chip-rol-si' : 'chip-rol-no'}" ${esMaster ? `class="editable-cell" onclick="editarMetaEmpleado('${e.id}', 'id_dispositivo', '${e.id_dispositivo || ''}')" title="Rol de Pagos"` : ''}>
                  <i class="fas fa-file-invoice-dollar"></i> ${e.id_dispositivo ? 'Con Rol' : 'Sin Rol'}
                </span>
                ${(e.esDesvinculado || (e.cargo || '').toLowerCase() === 'desvinculado' || (e.area || '').toLowerCase() === 'desvinculado') ?
                  `<span class="emp-chip chip-desvinculado" title="Colaborador Desvinculado"><i class="fas fa-user-slash"></i> Desvinculado ${(e.fecha_salida || e.fechaDesvinculacion) ? ' (' + (normalizarFechaStr(e.fecha_salida || e.fechaDesvinculacion) || e.fecha_salida) + ')' : ''}</span>` :
                  (e.esEliminado ? `<span class="emp-chip chip-eliminado" title="Colaborador Inactivo / Eliminado en base"><i class="fas fa-user-minus"></i> Inactivo en Base</span>` : '')
                }
                ${tardT > 0 ?
                  `<span class="emp-chip chip-warning" title="${tardT} tardanzas"><i class="fas fa-clock"></i> ${tardT} tardanzas</span>` :
                  `<span class="emp-chip chip-success" title="Sin atrasos"><i class="fas fa-check-circle"></i> Puntual</span>`
                }
              </div>
            </div>
          </div>
        ...
        <!-- BARRA INTEGRADA DE PERÍODO Y FILTRO DE FECHAS -->
        <div class="detail-period-toolbar">
          <div class="detail-period-selector-group">
            <span class="period-label"><i class="fas fa-calendar-alt" style="color:#4f46e5;"></i> Período:</span>
            <select id="filtroPeriodoDetalle" class="period-select" onchange="mostrarDetalle('${e.id}', parseInt(this.value))">
              ${optionsPeriodos}
            </select>
          </div>
          <div class="detail-date-range-group">
            <div class="date-field">
              <span class="date-label">Desde:</span>
              <input type="date" id="detFechaInicio" value="${R_INI}" onchange="mostrarDetalle('${e.id}', parseInt($('filtroPeriodoDetalle').value), this.value, $('detFechaFin').value)" class="date-input">
            </div>
            <div class="date-field">
              <span class="date-label">Hasta:</span>
              <input type="date" id="detFechaFin" value="${R_FIN}" onchange="mostrarDetalle('${e.id}', parseInt($('filtroPeriodoDetalle').value), $('detFechaInicio').value, this.value)" class="date-input">
            </div>
            <button type="button" class="btn-reset-period" onclick="mostrarDetalle('${e.id}', parseInt($('filtroPeriodoDetalle').value))" title="Restablecer al rango por defecto del período">
      ...
      <!-- SECCIÓN DE 4 MÉTRICAS EN GRID RESPONSIVO Y BALANCEADO -->
      <div class="kpi-grid-detail">
        <!-- 1. RESUMEN ASISTENCIA -->
        <div class="kpi-metric-card kpi-card-blue">
          <div class="kpi-header-row">
            <span class="kpi-header-title">
              <i class="fas fa-calendar-check" style="color:#2563eb; font-size:12px;"></i> Asistencia
            </span>
            <span class="emp-chip" style="background:#eff6ff; color:#1d4ed8; font-size:10px; padding:1px 6px;">Registro</span>
          </div>
          <div class="kpi-card-stats-grid">
            <div class="kpi-stat-item">
              <span class="kpi-stat-label">Días Trab.</span>
              <div class="kpi-stat-val-hero">${dias} <span style="font-size:10px; color:#94a3b8; font-weight:500;">(${entT}e/${salT}s)</span></div>
            </div>
            <div class="kpi-stat-item">
              <span class="kpi-stat-label">Puntualidad</span>
              <div class="kpi-stat-val-hero" style="color:${puntualidadColor};">${puntualidadVal}%</div>
            </div>
            <div class="kpi-stat-item">
              <span class="kpi-stat-label">Prom. Ent.</span>
              <div class="kpi-stat-val-sub" style="color:#334155;">${minsToHHMM(pE)}</div>
            </div>
            <div class="kpi-stat-item">
              <span class="kpi-stat-label">Prom. Sal.</span>
              <div class="kpi-stat-val-sub" style="color:#334155;">${minsToHHMM(pS)}</div>
            </div>
          </div>
        </div>

        <!-- 2. JORNADA Y TIEMPOS -->
        <div class="kpi-metric-card kpi-card-green">
          <div class="kpi-header-row">
            <span class="kpi-header-title">
              <i class="fas fa-clock" style="color:#10b981; font-size:12px;"></i> Jornada y Tiempos
            </span>
            <span class="emp-chip" style="background:#ecfdf5; color:#047857; font-size:10px; padding:1px 6px;">Laboral</span>
          </div>
          <div class="kpi-card-stats-grid">
            <div class="kpi-stat-item">
        ...
        <!-- 3. PERMISOS Y ALMUERZOS -->
        <div class="kpi-metric-card kpi-card-purple">
          <div class="kpi-header-row">
            <span class="kpi-header-title">
              <i class="fas fa-hand-holding-heart" style="color:#8b5cf6; font-size:12px;"></i> Permisos y Almuerzos
            </span>
            <span class="emp-chip" style="background:#faf5ff; color:#7e22ce; font-size:10px; padding:1px 6px;">Beneficios</span>
          </div>
          <div class="kpi-card-stats-grid">
            <div class="kpi-stat-item">
              <span class="kpi-stat-label">T. Personal</span>
              <div class="kpi-stat-val-sub" style="color:#6366f1;">${minutosAHHMMSS(totTP)}</div>
            </div>
            <div class="kpi-stat-item">
              <span class="kpi-stat-label">T. Médico</span>
              <div class="kpi-stat-val-sub" style="color:#0d9488;">${minutosAHHMMSS(totTM)}</div>
            </div>
            <div class="kpi-stat-item">
              <span class="kpi-stat-label">Alm. Planta</span>
              <div class="kpi-stat-val-sub" style="color:#8b5cf6;">${almP}d</div>
            </div>
            <div class="kpi-stat-item">
              <span class="kpi-stat-label">Alm. Fuera</span>
              <div class="kpi-stat-val-sub" style="color:#64748b;">${almF}d</div>
            </div>
          </div>
        </div>

        <!-- 4. HISTORIAL DE VACACIONES -->
        <div id="card-vacaciones-detalle" class="kpi-metric-card kpi-card-amber">
          <div class="kpi-header-row">
            <span class="kpi-header-title">
              <i class="fas fa-umbrella-beach" style="color:#d97706; font-size:12px;"></i> Vacaciones
            </span>
            <span class="emp-chip" style="background:#fffbeb; color:#b45309; font-size:10px; padding:1px 6px;">Saldo</span>
          </div>
          <div style="display:flex; align-items:center; justify-content:center; padding:12px 0; color:#64748b; font-size:11px; gap:6px; min-height:48px;">
        ...
        <div class="table-wrapper">
          <div class="table-scroll-wrap">
            <table class="employee-table table-compact">
              <thead>
                <tr>
                  <th style="font-size:11px; padding:6px 8px; text-align:left;">Fecha</th>
                  <th style="font-size:11px; padding:6px 8px; text-align:center;">Entrada</th>
                  <th style="font-size:11px; padding:6px 8px; text-align:center;">Salida</th>
                  <th style="font-size:11px; padding:6px 8px; text-align:center;" title="Total Horas Trabajadas">Horas Trab.</th>
                  <th style="font-size:11px; padding:6px 8px; text-align:center;" title="Modalidad de Almuerzo (Planta / Fuera)">Almuerzo</th>
                  <th style="font-size:11px; padding:6px 8px; text-align:center;" title="Atrasos, Salidas Anticipadas, Justificaciones, Permisos y Observaciones">Novedades & Permisos</th>
                  <th style="font-size:11px; padding:6px 8px; text-align:center; color:var(--red);" title="Tiempo neto a descontar">T. Descontar</th>
                  <th style="font-size:11px; padding:6px 8px; text-align:center; color:#1d4ed8;" title="Horas Extra 50% con estado de autorización">H.E. 50%</th>
                  <th style="font-size:11px; padding:6px 8px; text-align:center; color:#4338ca;" title="Horas Extra 100% con estado de autorización">H.E. 100%</th>
                </tr>
              </thead>
              <tbody id="tbody-historial-periodo">${filas || '<tr><td colspan="9" class="empty-state">Sin registros</td></tr>'}</tbody>
...
// Extraido a: JS/supervisor/supervisor_directorio.js
// ============================================================
// ============================================================
...
async function completarRegistro(empleadoId, tipo, hora, fecha) {
  mostrarLoader(true);
  try {
    let fParts = (fecha || '').includes('-') ? fecha.split('-') : fecha.split('/').reverse();
    let y = fParts[0], mo = fParts[1], d = fParts[2];
    ...
    const res = await jsonpRequest({
      ...
      docId: `${empleadoId}_${tipo}_${fecha}_${hVal.replace(/:/g, '')}`,
      empleadoId: empleadoId,
      tipo: tipo,
      fecha: fecha,
      campo: 'timestamp',
...
window.mostrarModalFuturos = function (preselectedEmpId = null, preselectedFecha = null) {
  ...
  const fechaDefecto = preselectedFecha || hoy;
  $('futFechaInicio').value = fechaDefecto;
  $('futFechaFin').value = fechaDefecto;
...
window.guardarEventoFuturo = async function () {
  ...
  if (!eid) {
    mostrarToast('Seleccione un empleado', 'error');
    return;
  }
  if (!fInicio || !fFin) {
  ...
  try {
    ...
    for (const f of fechas) {
      ...
      if (res && res.ok) {
        successCount++;
        // Actualizar caché en memoria del empleado inmediatamente
        if (emp) {
          if (!emp.registros) emp.registros = [];
          emp.registros = emp.registros.filter(r => !(r.fecha === f && ['VACACIONES', 'PERMISO_PERSONAL', 'PERMISO_MEDICO', 'SALIDA_JUSTIFICADA', 'FALTA_JUSTIFICADA'].includes((r.tipo || '').toUpperCase())));
          emp.registros.push({
            id: `${eid}_${tipo}_${f}_000000`,
            empleadoId: eid,
            nombre: emp.nombre || '',
            fecha: f,
            hora: '00:00:00',
    ...
    if (successCount === fechas.length) {
      ...
      await cargarDatosCompletos(true, true);
      if (panelActual === 'detalle') {
        mostrarDetalle(eid, 0, null, null, fInicio);
      } else {
        cargarAsistencia();
        if (typeof renderizarMapaCalorAsistencia === 'function') renderizarMapaCalorAsistencia();
      }
    } else {
    ...
    mostrarToast('Error de conexión al registrar eventos futuros: ' + e.message, 'error');
  } finally {
    mostrarLoader(false);
  }
}
...
window.editarValorRegistro = async function (empleadoId, tipo, docId, campo, valorActual, fecha) {
  ...
  if (campo === 'hora') {
    ...
    let y = fParts[0], mo = fParts[1], d = fParts[2];
    let hVal = nuevoValor && nuevoValor.length === 5 ? nuevoValor + ':00' : nuevoValor;
    if (y && mo && d && hVal) {
      campoRequest = 'timestamp';
      valorRequest = `${String(d).padStart(2, '0')}/${String(mo).padStart(2, '0')}/${y} ${hVal}`;
    }
  }
  ...
  if (emp) {
    ...
    if (reg) {
      if (campo === 'timestamp') {
        ...
        reg.timestamp = valorRequest;
        reg.hora = nuevoValor && nuevoValor.length === 5 ? nuevoValor + ':00' : nuevoValor;
      ...
      let newReg = {
        ...
        fecha: targetFecha,
        tipo: tipo,
        hora: campo === 'hora' ? (nuevoValor.length === 5 ? nuevoValor + ':00' : nuevoValor) : '00:00:00',
        ...
        timestamp: valorRequest || new Date().toISOString()
      };
      if (campo === 'timestamp') {
        const parsed = parsearTimestamp(nuevoValor);
        if (parsed) {
        ...
        newReg.timestamp = valorRequest;
        if (typeof window.normalizarRegistroDesdeTimestamp === 'function') window.normalizarRegistroDesdeTimestamp(newReg);
      }
  ...
  try {
    ...
    empCache.forEach(emp => {
      if (emp.registros && emp.registros.length) {
        emp.registros.forEach(r => {
          if (typeof window.normalizarRegistroDesdeTimestamp === 'function') window.normalizarRegistroDesdeTimestamp(r);
...
// Extraido a: JS/supervisor/supervisor_reportes_custom.js
// ============================================================
// ============================================================
...
// ============================================================
// CONTROL DE EMERGENCIAS Y SIMULACROS EN SUPERVISOR
// Extraido a: JS/supervisor/supervisor_emergencias.js
// ============================================================
// ============================================================
...
window.renderizarTablaDesgloseVacaciones = function () {
  try {
    ...
    console.error('Error en renderizarTablaDesgloseVacaciones:', e);
  }
};
...
window.abrirModalDesgloseVacaciones = function (forzarRecarga = false) {
  try {
    ...
    // 1. Intentar leer de localStorage si la memoria está vacía
    if (!window.kpiVacacionesIndividual || Object.keys(window.kpiVacacionesIndividual).length === 0) {
      try {
    ...
    const tieneDatos = window.kpiVacacionesIndividual && Object.keys(window.kpiVacacionesIndividual).length > 0;

    // Si ya tenemos datos y no se forzó recarga, renderizar de inmediato sin mostrar spinner
    if (tieneDatos && !forzarRecarga) {
      window.renderizarTablaDesgloseVacaciones();
      return;
    }

    // Si no tenemos datos, mostrar spinner de carga
    if (!tieneDatos) {
      if (tbody) {
      ...
      // Si forzó recarga pero ya tenemos datos, renderizar los actuales mientras se refresca
      window.renderizarTablaDesgloseVacaciones();
    }

    window.sincronizarDatosVacaciones(false);
  } catch (err) {
...
window.sincronizarDatosVacaciones = function (abrirModalDespues = false, btn = null) {
  ...
  // Si ya hay una sincronización en curso, reusar la misma promesa para evitar duplicados
  if (window._sincronizandoVacacionesPromise) {
  ...
  window._sincronizandoVacacionesPromise = reqVac.then(vacRes => {
    if (vacRes && vacRes.ok && vacRes.kpiVacacionesIndividual && Object.keys(vacRes.kpiVacacionesIndividual).length > 0) {
      ...
      // Renderizar modal sin volver a disparar sincronización (evita bucles infinitos)
      window.renderizarTablaDesgloseVacaciones();
      ...
      // Si la respuesta falló o no trajo datos individuales, revisar si hay caché previa
      const hayCache = window.kpiVacacionesIndividual && Object.keys(window.kpiVacacionesIndividual).length > 0;
      if (hayCache) {
        window.renderizarTablaDesgloseVacaciones();
        if (btn && typeof mostrarToast === 'function') {
      ...
      throw new Error((vacRes && vacRes.error) || 'El servidor de Google Sheets tardó demasiado en responder.');
    }
  }).catch(err => {
    ...
    if (tbody) {
      tbody.innerHTML = `<tr><td colspan="8" style="padding: 25px; text-align: center; color: #ef4444;">
        <i class="fas fa-exclamation-triangle" style="font-size: 22px; margin-bottom: 8px; display: block;"></i>
        <div style="font-weight: 600; margin-bottom: 6px;">No se pudieron sincronizar las vacaciones desde Google Sheets.</div>
        <div style="font-size: 11px; color: #64748b; margin-bottom: 12px;">${(typeof escapeHtml === 'function') ? escapeHtml(err.message || 'Tiempo de espera agotado') : (err.message || 'Tiempo de espera agotado')}</div>
        <button type="button" onclick="window.sincronizarDatosVacaciones(true)" class="btn" style="padding: 5px 14px; font-size: 11.5px; background: #fee2e2; border: 1px solid #fca5a5; color: #b91c1c; border-radius: 6px; cursor: pointer; font-weight: 700;">
          <i class="fas fa-sync-alt" style="margin-right: 4px;"></i> Reintentar sincronización
        </button>
      </td></tr>`;
    }
    if (btn && typeof mostrarToast === 'function') {
    ...
    window._sincronizandoVacacionesPromise = null;
    if (btn) {
...
window.esCumpleanosEmpleadoEnFecha = function (emp, fechaStr) {
  ...
  const rawN = (typeof window.obtenerFechaNacimientoEmpleado === 'function') 
    ? window.obtenerFechaNacimientoEmpleado(emp) 
    : (emp.fechaNacimiento || emp.fecha_nacimiento || emp.fNacimiento || emp.fechanacimiento || '');
  if (!rawN) return false;
  const normNac = (typeof window.normalizarFechaParaInput === 'function') 
    ? window.normalizarFechaParaInput(rawN) 
    : String(rawN).split('T')[0];
  const normFecha = (typeof window.normalizarFechaParaInput === 'function') 
    ? window.normalizarFechaParaInput(fechaStr) 
    : String(fechaStr).split('T')[0];
  if (!normNac || !normFecha) return false;
  const pNac = normNac.split('-');
  const pFecha = normFecha.split('-');
...
window.actualizarEstadoSeccionRazon = function () {
  ...
  const seccion2 = document.getElementById('seccionModalJornadaRazon');
  ...
  const avisoInactivo = document.getElementById('modalJornadaAvisoSeccion2Inactiva');

  if (tieneHoras) {
    if (seccion2) {
      seccion2.style.opacity = '0.8';
      seccion2.style.background = '#f8fafc';
    }
    if (razonSel) {
      razonSel.disabled = false;
      razonSel.style.cursor = 'pointer';
      razonSel.style.background = '#f1f5f9';
      razonSel.style.borderColor = '#cbd5e1';
      razonSel.style.color = '#64748b';
    }
    if (inputOtro) inputOtro.disabled = false;
    if (avisoInactivo) avisoInactivo.style.display = 'flex';
  } else {
    if (seccion2) {
      seccion2.style.opacity = '1';
      seccion2.style.background = '#ffffff';
    }
    if (razonSel) {
      razonSel.disabled = false;
      razonSel.style.cursor = 'pointer';
      razonSel.style.background = '#f5f3ff';
      razonSel.style.borderColor = '#8b5cf6';
      razonSel.style.color = '#5b21b6';
    }
    if (inputOtro) inputOtro.disabled = false;
    if (avisoInactivo) avisoInactivo.style.display = 'none';
  }
};
...
window.abrirModalGestionJornada = function (empleadoId, fecha) {
  ...
  const poolEmps = (typeof empCache !== 'undefined' && empCache && empCache.length) ? empCache : (window.empCache || []);
  let emp = null;
  if (idBuscado) {
    emp = poolEmps.find(x => String(x.id).trim() === idBuscado || String(x.cedula || '').trim() === idBuscado)
  ...
  if (!emp && window.idDetalleActual) {
    ...
    emp = poolEmps.find(x => String(x.id).trim() === idDet || String(x.cedula || '').trim() === idDet);
  }
  ...
  let netWorkedOrdinario = (typeof calcularNetWorkedOrdinario === 'function') ? calcularNetWorkedOrdinario(periodosDia, esFestivo) : netWorked;
  let faltanteJornada = esFestivo ? 0 : Math.max(0, 480 - netWorkedOrdinario);

  // Los 45 min de almuerzo son derecho del usuario y neutros: no computan como falta ni atraso
  if (!esFestivo && atrasoMins > faltanteJornada) {
    atrasoMins = faltanteJornada;
  }
  ...
  const medMins = regPermiso ? Number(regPermiso.permiso_medico_mins || 0) : 0;
  const justMins = regPermiso ? Number(regPermiso.tiempo_justificado_mins || 0) : 0;
  let comentario = regPermiso ? (regPermiso.razon_permiso || '') : '';
  ...
  // Identificar marcaciones y asistencia
  ...
  const tieneAsistencia = regsDia.some(r => ['ENTRADA', 'SALIDA', 'RETORNO_CAMPO', 'SALIDA_CAMPO', 'ENTRADA_CAMPO'].includes(String(r.tipo || '').toUpperCase()))
    || Boolean(primerEntrada || ultimoSalida)
    || (netWorked > 0);

  // Detección automática de cumpleaños según fecha
  const esCumpleHoy = window.esCumpleanosEmpleadoEnFecha(emp, fechaEfectiva);
  ...
  window._modalJornadaContexto = {
    ...
    tieneAsistencia,
    esCumpleHoy,
    minsJustificado: justMins,
    minsPersonal: persMins,
    minsMedico: medMins,
    almuerzoActual: almuerzoVal,
    horasExtrasActual: heVal,
    observacionActual: comentario,
    razonActual: razonActual
  };

  // Asignar al DOM
  document.getElementById('modalJornadaColaboradorNombre').textContent = emp.nombre || `ID: ${empleadoId}`;
  const fParts = fechaEfectiva.split('-');
  const diaNom = (typeof obtenerDiaSemanaStr === 'function') ? obtenerDiaSemanaStr(fechaEfectiva) : '';
  ...
  // Banner automático de Cumpleaños
  const bannerCumple = document.getElementById('modalJornadaBannerCumpleanos');
  if (bannerCumple) {
    bannerCumple.style.display = esCumpleHoy ? 'flex' : 'none';
  }

  // Inputs de horas: extraídos directamente desde TIMESTAMP como fuente de verdad
  document.getElementById('modalJornadaHoraEntrada').value = primerEntrada ? (formatearHora(primerEntrada.timestamp || primerEntrada.hora) || '') : '';
  document.getElementById('modalJornadaHoraSalida').value = ultimoSalida ? (formatearHora(ultimoSalida.timestamp || ultimoSalida.hora) || '') : '';
  ...
  // Habilitar opciones de día completo
  const optVac = document.getElementById('modalOptVacacion');
  const optFer = document.getElementById('modalOptFeriado');
  const optInas = document.getElementById('modalOptInasistencia');
  if (optVac) optVac.disabled = false;
  if (optFer) optFer.disabled = false;
  if (optInas) optInas.disabled = false;

  // Razón de ausencia
  const razonSel = document.getElementById('modalJornadaRazonSelect');
  const divOtro = document.getElementById('modalJornadaDivRazonPersonalizada');
  const inputOtro = document.getElementById('modalJornadaRazonOtroInput');
  const opcionesValidas = ['Vacación', 'Feriado', 'Permiso Médico', 'Permiso Personal', 'Falta Justificada', 'Campo', 'Inasistencia'];

  if (razonSel) {
    if (primerEntrada || ultimoSalida) {
      razonSel.value = '';
      if (divOtro) divOtro.style.display = 'none';
      if (inputOtro) inputOtro.value = '';
    } else if (opcionesValidas.includes(razonActual)) {
      razonSel.value = razonActual;
      if (divOtro) divOtro.style.display = 'none';
    } else if (razonActual && razonActual !== 'Cumpleaños') {
      razonSel.value = 'Otro';
  ...
  // Asignar tiempos existentes directamente sin restricciones artificiales
  let cleanJust = justMins;
  let cleanPers = persMins;
  let cleanMed = medMins;

  // Si es cumpleaños del colaborador y tiene horas trabajadas con faltante/atraso, aplicar beneficio institucional automático
  if (esCumpleHoy) {
    if (tieneAsistencia && cleanJust === 0 && cleanPers === 0 && cleanMed === 0 && faltanteJornada > 0) {
      cleanJust = 240;
      if (!comentario) comentario = 'Beneficio institucional por cumpleaños (4 horas)';
    } else if (!tieneAsistencia && !razonActual) {
      if (!comentario) comentario = 'Día de cumpleaños del colaborador';
    }
  }

  const elJust = document.getElementById('modalJornadaMinsJustificado');
  const elPers = document.getElementById('modalJornadaMinsPersonal');
  const elMed = document.getElementById('modalJornadaMinsMedico');
  if (elJust) { elJust.max = 720; elJust.value = cleanJust; }
  if (elPers) { elPers.max = 720; elPers.value = cleanPers; }
  if (elMed) { elMed.max = 720; elMed.value = cleanMed; }
  ...
  // Actualizar estado activo/inactivo de la Sección 2
  window.actualizarEstadoSeccionRazon();

  // Banner de ayuda dinámica según razón inicial
  window.alCambiarRazonModalJornada(razonSel?.value || '', true);
  ...
  const fInicioModalEmp = (typeof window.obtenerFechaInicioEfectivaEmpleado === 'function')
    ? window.obtenerFechaInicioEfectivaEmpleado(emp, fechaEfectiva)
    : (emp.fecha_ingreso || fechaEfectiva);
  if (fechaEfectiva < fInicioModalEmp && (!razonActual || razonActual === '')) {
    ...
    if (bannerAyuda) {
      ...
      bannerAyuda.style.border = '1.5px solid #cbd5e1';
      bannerAyuda.style.color = '#475569';
      bannerAyuda.innerHTML = `<i class="fas fa-info-circle" style="font-size:20px; color:#64748b;"></i> <div><strong>Usuario recién registrado:</strong> Esta jornada es anterior a la fecha de ingreso/registro del colaborador (${fInicioModalEmp}). No corresponde a una inasistencia injustificada.</div>`;
    }
  }
...
window.actualizarEtiquetasMinutosModal = function (origen) {
  const elJust = document.getElementById('modalJornadaMinsJustificado');
  const elPers = document.getElementById('modalJornadaMinsPersonal');
  const elMed = document.getElementById('modalJornadaMinsMedico');

  let just = parseInt(elJust?.value || '0') || 0;
  let pers = parseInt(elPers?.value || '0') || 0;
  let med = parseInt(elMed?.value || '0') || 0;

  // Límite técnico máximo de 720 min (12 horas) por categoría
  if (just > 720) { just = 720; if (elJust) elJust.value = 720; }
  if (pers > 720) { pers = 720; if (elPers) elPers.value = 720; }
  if (med > 720) { med = 720; if (elMed) elMed.value = 720; }
  ...
  const total = just + pers + med;
  if (totalTxt) {
    totalTxt.textContent = `Total Permisos: ${fnFormat(total)} (${total} min)`;
    totalTxt.style.color = '#15803d';
  }
};
...
window.recalcularTiemposModalJornada = function () {
  ...
  if (hE && hS && typeof obtenerMinutos === 'function') {
    ...
    if (mE !== null && mS !== null && mS > mE) {
      ...
      if (!esFestivo && ordDur > 240) ordDur -= 45;
      ordDur = Math.max(0, ordDur);
      const falt = esFestivo ? 0 : Math.max(0, 480 - ordDur);

      let atraso = (mE > refEntrada + 5) ? (mE - refEntrada) : 0;
      if (!esFestivo && atraso > falt) {
        atraso = falt;
      }

      if (ctx) {
        ctx.netWorked = dur;
        ctx.netWorkedOrdinario = ordDur;
        ctx.faltanteJornada = falt;
        ctx.atrasoMins = atraso;
        ctx.tieneAsistencia = true;
      }
      document.getElementById('modalJornadaInfoNeto').textContent = (typeof minutosAHHMMSS === 'function') ? minutosAHHMMSS(dur) : `${dur} min`;
      document.getElementById('modalJornadaInfoAtraso').textContent = (typeof minutosAHHMMSS === 'function') ? (atraso > 0 ? minutosAHHMMSS(atraso) : '00:00:00') : `${atraso} min`;
      document.getElementById('modalJornadaInfoFaltante').textContent = (typeof minutosAHHMMSS === 'function') ? (falt > 0 ? minutosAHHMMSS(falt) : '00:00:00') : `${falt} min`;
  ...
  if (!hE && !hS && ctx) {
    ctx.netWorked = 0;
    ctx.netWorkedOrdinario = 0;
    ctx.atrasoMins = 0;
    ctx.faltanteJornada = 0;
    ctx.tieneAsistencia = false;
    document.getElementById('modalJornadaInfoNeto').textContent = '00:00:00';
    document.getElementById('modalJornadaInfoAtraso').textContent = '00:00:00';
    document.getElementById('modalJornadaInfoFaltante').textContent = '00:00:00';
  }
  window.actualizarEstadoSeccionRazon();
};

window.alCambiarRazonModalJornada = function (valor, esCargaInicial = false) {
  const banner = document.getElementById('modalJornadaBannerAyuda');
  const divOtro = document.getElementById('modalJornadaDivRazonPersonalizada');
  if (divOtro) divOtro.style.display = (valor === 'Otro') ? 'block' : 'none';

  if (valor && !esCargaInicial) {
    // Si el usuario selecciona un motivo de ausencia de día completo, limpiar horas
    const inputE = document.getElementById('modalJornadaHoraEntrada');
    const inputS = document.getElementById('modalJornadaHoraSalida');
    if (inputE && inputE.value) inputE.value = '';
    if (inputS && inputS.value) inputS.value = '';
    ...
    const infoNeto = document.getElementById('modalJornadaInfoNeto');
    if (infoNeto) infoNeto.textContent = '00:00:00';
    const infoAtraso = document.getElementById('modalJornadaInfoAtraso');
    if (infoAtraso) infoAtraso.textContent = '00:00:00';
    const infoFaltante = document.getElementById('modalJornadaInfoFaltante');
    if (infoFaltante) infoFaltante.textContent = '00:00:00';
  }
  ...
  if (valor === 'Vacación') {
    ...
    banner.style.border = '1.5px solid #99f6e4';
    banner.style.color = '#0f766e';
    banner.innerHTML = `<i class="fas fa-umbrella-beach" style="font-size:20px; color:#0d9488;"></i> <div><strong>Vacación:</strong> Goce de vacación de jornada completa autorizada.</div>`;
  } else if (valor === 'Feriado') {
    banner.style.background = '#eef2ff';
    banner.style.border = '1.5px solid #c7d2fe';
    banner.style.color = '#3730a3';
    banner.innerHTML = `<i class="fas fa-landmark" style="font-size:20px; color:#6366f1;"></i> <div><strong>Feriado:</strong> Descanso obligatorio oficial según calendario nacional.</div>`;
    ...
    banner.innerHTML = `<i class="fas fa-stethoscope" style="font-size:20px; color:#0284c7;"></i> <div><strong>Permiso Médico Completo:</strong> Jornada completa no laborada justificada por cita médica o reposo.</div>`;
    ...
    banner.innerHTML = `<i class="fas fa-user-times" style="font-size:20px; color:#7c3aed;"></i> <div><strong>Permiso Personal Completo:</strong> Jornada completa no laborada autorizada por asuntos personales.</div>`;
  } else if (valor === 'Falta Justificada') {
    banner.style.background = '#ecfdf5';
    banner.style.border = '1.5px solid #a7f3d0';
    banner.style.color = '#065f46';
    banner.innerHTML = `<i class="fas fa-check-circle" style="font-size:20px; color:#059669;"></i> <div><strong>Falta Justificada:</strong> Ausencia de jornada completa aprobada por el supervisor.</div>`;
  } else if (valor === 'Campo') {
    banner.style.background = '#fffbeb';
    banner.style.border = '1.5px solid #fde68a';
    banner.style.color = '#92400e';
    banner.innerHTML = `<i class="fas fa-truck" style="font-size:20px; color:#d97706;"></i> <div><strong>Trabajo de Campo:</strong> Jornada completa en labores externas o de campo.</div>`;
  } else if (valor === 'Inasistencia') {
    banner.style.background = '#fff1f2';
    banner.style.border = '1.5px solid #fecdd3';
    banner.style.color = '#be123c';
    banner.innerHTML = `<i class="fas fa-calendar-times" style="font-size:20px; color:#e11d48;"></i> <div><strong>Inasistencia Injustificada:</strong> Falta de día completo sin justificación sujeta a descuento de nómina.</div>`;
  } else {
    banner.style.display = 'none';
  }
};
...
window.aplicarAccionRapidaModal = function (accion) {
  const ctx = window._modalJornadaContexto;
  if (!ctx) return;
  const faltante = ctx.faltanteJornada || 0;
  ...
  if (accion === 'completar_8h') {
    if (elJust) elJust.value = faltante;
    if (elPers) elPers.value = 0;
    if (elMed) elMed.value = 0;
  } else if (accion === 'cubrir_atraso_medico') {
    const minCubrir = atraso > 0 ? (faltante > 0 ? Math.min(atraso, faltante) : atraso) : (faltante > 0 ? faltante : 60);
    if (elMed) elMed.value = minCubrir;
    if (elJust) elJust.value = 0;
    if (elPers) elPers.value = 0;
  } else if (accion === 'cubrir_atraso_personal') {
    const minCubrir = atraso > 0 ? (faltante > 0 ? Math.min(atraso, faltante) : atraso) : (faltante > 0 ? faltante : 60);
    if (elPers) elPers.value = minCubrir;
    if (elJust) elJust.value = 0;
    if (elMed) elMed.value = 0;
...
window.guardarModalGestionJornada = async function () {
  const ctx = window._modalJornadaContexto;
  if (!ctx) return;
  const { empleadoId, fecha, emp, esCumpleHoy } = ctx;
  ...
  let razonFinal = (razonSel === 'Otro') ? (inputOtro || 'Otro') : razonSel;
  if (!razonFinal && esCumpleHoy && !hEntrada && !hSalida) {
    razonFinal = 'Cumpleaños';
  }

  const tieneHoras = Boolean(hEntrada || hSalida);
  const esAusenciaCompleta = !tieneHoras && Boolean(razonFinal);
  const totalPermisos = minsJust + minsPers + minsMed;

  // 1. VALIDACIÓN: Límite técnico de 720 min por categoría
  if (minsJust > 720 || minsPers > 720 || minsMed > 720) {
    if (typeof mostrarToast === 'function') {
      mostrarToast('❌ El tiempo asignado a un permiso no puede superar las 12 horas (720 min).', 'error');
    }
    return;
  }

  // 2. VALIDACIÓN: Si no tiene horas ni permisos, debe tener una razón de ausencia
  if (!tieneHoras && !razonFinal && totalPermisos === 0) {
    if (typeof mostrarToast === 'function') {
      mostrarToast('⚠️ Ingrese horas de asistencia o seleccione una razón de ausencia en la Sección 2.', 'warn');
    }
    return;
  }
  ...
  try {
    ...
    if (esAusenciaCompleta) {
      ...
      if (emp) {
        if (!emp.registros) emp.registros = [];
        // Limpiar marcaciones previas del día al convertir a ausencia de día completo
      ...
      // Limpiar registros virtuales de ausencia previa del día
    ...
    // TAREAS DE SINCRONIZACIÓN EN SEGUNDO PLANO
    const backgroundTasks = [];
    const localHoy = (typeof getLocalHoyStr === 'function') ? getLocalHoyStr() : new Date().toISOString().split('T')[0];
    const esFechaPasada = (fecha < localHoy);
    const yaArchivado = Boolean((ctx.regsDia || []).some(r => String(r.id || '').startsWith('arch_') || r.archivado));

    if (esAusenciaCompleta) {
      // Purgar marcaciones huérfanas en Firestore para esa fecha siempre
      if (window.FirebaseBackend && window.FirebaseBackend.eliminarRegistroFirestorePorFecha) {
      ...
      if (esFechaPasada || yaArchivado) {
        ...
        // Fecha actual / futura: guardar en Firestore
        if (window.FirebaseBackend && window.FirebaseBackend.guardarRegistro) {
      ...
      // Sincronizar Permisos Parciales de manera independiente (Sección 3)
      const tiposPermisos = [
        { tipo: 'justificado', valor: minsJust, previo: ctx.minsJustificado || 0 },
        { tipo: 'personal', valor: minsPers, previo: ctx.minsPersonal || 0 },
        { tipo: 'medico', valor: minsMed, previo: ctx.minsMedico || 0 }
      ];

      for (const p of tiposPermisos) {
        if (p.valor > 0 || p.previo > 0) {
          const permParams = {
            empleadoId,
            fecha,
            tipo: p.tipo,
          ...
          if (window.FirebaseBackend && window.FirebaseBackend.guardarPermisoSupervisor) {
            backgroundTasks.push(window.FirebaseBackend.guardarPermisoSupervisor(permParams).catch(err => console.warn(`Aviso permiso ${p.tipo}:`, err)));
          } else if (typeof jsonpRequest === 'function') {
            backgroundTasks.push(jsonpRequest({ accion: 'guardarPermisoSupervisor', ...permParams }).catch(err => console.warn(`Aviso Sheets permiso ${p.tipo}:`, err)));
          }
        }
      }
      ...
      // Almuerzo u Horas Extras si no se enviaron horas en la entrada
      if (!hEntrada) {
        if (almuerzoVal && almuerzoVal !== (ctx.almuerzoActual || 'NO')) {
          if (window.FirebaseBackend && window.FirebaseBackend.actualizarAlmuerzoSupervisor) {
            backgroundTasks.push(window.FirebaseBackend.actualizarAlmuerzoSupervisor({ empleadoId, almuerzo: almuerzoVal, fecha }).catch(err => console.warn("Aviso almuerzo modal:", err)));
          }
        }
        if (horasExtrasVal && horasExtrasVal !== (ctx.horasExtrasActual || 'NO')) {
          if (window.FirebaseBackend && window.FirebaseBackend.actualizarRegistroGeneral) {
            backgroundTasks.push(window.FirebaseBackend.actualizarRegistroGeneral({
              empleadoId,
              fecha,
              tipo: 'ENTRADA',
              campo: 'horasExtra',
              valor: horasExtrasVal
            }).catch(err => console.warn("Aviso extras modal:", err)));
          }
        }
      }
    }
...
window.eliminarMarcacionesDiaModal = async function () {
  ...
  try {
    ...
    if (storedArch) {
      ...
      if (archData && Array.isArray(archData.registros)) {
        archData.registros = archData.registros.filter(r => r.fecha !== fecha);
  ...
  // 3. Tareas en segundo plano paralelas (Firestore + Sheets)
  const delTasks = [];

  // Purgar en Firestore
  if (window.FirebaseBackend && window.FirebaseBackend.eliminarRegistroFirestorePorFecha) {
    delTasks.push(window.FirebaseBackend.eliminarRegistroFirestorePorFecha(empleadoId, fecha).catch(e => console.warn("Aviso Firestore eliminar:", e)));
  }
