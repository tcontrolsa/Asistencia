...
const file = path.join(baseDir, 'api_completa.gs');

if (fs.existsSync(file)) {
  const content = fs.readFileSync(file, 'utf8');
  const lines = content.split('\n');
  console.log(`Searching API Completa...`);
  lines.forEach((line, idx) => {
    if (line.includes('HORA') || line.includes('ENTRADA') || line.includes('SALIDA') || line.includes('festivo') || line.includes('feriado') || line.includes('domingo') || line.includes('sabado') || line.includes('420') || line.includes('900')) {
      if (line.includes('function') || line.includes('const') || line.includes('let') || line.includes('var') || line.includes('if') || line.includes('else') || line.includes('?')) {
