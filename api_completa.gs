...
function procesarAccion(params) {
  ...
  switch(accion) {
    ...
    case 'obtenerPreguntasCultura':
      return obtenerPreguntasCultura();
...
function actualizarPerfilEmpleado(params) {
  ...
  try {
    ...
    if (params.passwordHash !== undefined || params.pin !== undefined) {
      const pHash = String(params.passwordHash !== undefined ? params.passwordHash : params.pin).trim();
...
function actualizarAlmuerzoSupervisor(params) {
  ...
  try {
    ...
    if (filaEncontrada === -1) {
      // Si el parámetro soloSiExiste es verdadero o el empleado es de cargo "Solo Almuerzo" / "Sin Asistencia",
      // no creamos fila en Sheets para no duplicar con Firebase
      const infoEmpleado = obtenerInfoEmpleado(empleadoId);
      const cargo = (infoEmpleado.cargo || '').toString().trim().toUpperCase();
      const esSoloAlm = cargo === 'SOLO ALMUERZO' || cargo === 'SOLO_ALMUERZO' || cargo === 'SIN ASISTENCIA';
      
      if (params.soloSiExiste || esSoloAlm) {
        return { ok: true, mensaje: "Omitida inserción en Sheets por ser Solo Almuerzo / Gestionado en Firebase" };
      }
...
function obtenerPreguntasCultura() {
  try {
    var ss = SpreadsheetApp.getActiveSpreadsheet();
    var sheet = ss.getSheetByName("CULTURA_PREGUNTAS");
    if (!sheet) {
    ...
    for (var i = 1; i < dataRange.length; i++) {
      ...
      var opciones = [];
      try {
        opciones = r[6] ? JSON.parse(String(r[6])) : [];
      } catch(e) {}
      preguntas.push({
        id: id,
        tipo: r[1] ? String(r[1]).trim() : 'PROPOSITO',
        pilar: r[2] ? String(r[2]).trim() : 'Propósito',
        iconoPilar: r[3] ? String(r[3]).trim() : '🎯',
        clasePilar: 'quiz-pillar-' + (r[1] ? String(r[1]).toLowerCase() : 'proposito'),
        pregunta: r[4] ? String(r[4]).trim() : '',
        pista: r[5] ? String(r[5]).trim() : '',
        opciones: opciones,
        activo: r[7] !== false && String(r[7]).toUpperCase() !== 'NO'
      });
    }
    return { ok: true, preguntas: preguntas };
  } catch(e) {
    return { error: e.toString() };
  }
}

function guardarPreguntasCultura(params) {
  try {
    var ss = SpreadsheetApp.getActiveSpreadsheet();
    var sheet = ss.getSheetByName("CULTURA_PREGUNTAS");
    if (!sheet) {
      sheet = ss.insertSheet("CULTURA_PREGUNTAS");
      sheet.getRange(1, 1, 1, 9).setValues([["ID", "TIPO", "PILAR", "ICONO", "PREGUNTA", "PISTA", "OPCIONES_JSON", "ACTIVO", "FECHA_ACTUALIZACION"]]);
      sheet.getRange(1, 1, 1, 9).setFontWeight("bold").setBackground("#fee2e2");
    }
    
    var raw = params.preguntas;
    var preguntas = typeof raw === 'string' ? JSON.parse(raw) : raw;
    if (!Array.isArray(preguntas)) {
      return { error: "El formato de preguntas debe ser un arreglo" };
    }
    
    // Limpiar contenido previo a partir de la fila 2
    ...
    return { ok: true, total: preguntas.length, mensaje: "Preguntas guardadas en Google Sheets exitosamente" };
  } catch(e) {
    return { error: e.toString() };
  }
}
